...
function updateFilterButtonsFromState() {
    Object.entries(btnElements).forEach(([key, btn]) => {
        const id = typeof btn.dataset.id === 'string' && /^\d+$/.test(btn.dataset.id) ? parseInt(btn.dataset.id, 10) : btn.dataset.id;
...
async function register() {
    ...
    if (res.ok) {
        let message = "Registered! Please check your email for the token.";
        if (data.token) {
            message = `Registered! Use this token to verify: ${data.token}`;
        }
        showToast(message);
        switchAuthTab('login'); // Switch back to login view so they see the token input box
    } else {
...
async function fetchRecommendations() {
    ...
    if (!currentUser) {
        toggleModal(true);
        showToast("You must be logged in to discover anime!", true);
        return;
    }
    ...
    try {
        ...
        showToast("Error fetching data. Check terminal.", true);
    } finally {
...
function sortAndRender() {
    ...
    if (sortBy === 'match') {
        ...
        sortedData.sort((a, b) => (b.watching + b.completed + b.dropped + b.plan) - (a.watching + a.completed + a.dropped + a.plan));
    }
...
function exportResults() {
    if (!currentResults || currentResults.length === 0) {
        showToast('No recommendations available to export.', true);
        return;
    }
    const data = currentResults.map(anime => ({
        ...
        id: anime.id,
        score: anime.score,
        raw_mean_score: anime.raw_mean_score,
        genres: anime.genres,
        status: anime.status,
...
function renderResults(animes) {
    ...
    animes.forEach((anime, i) => {
        ...
        const total = anime.watching + anime.completed + anime.dropped + anime.plan;
        const wPct = (anime.watching / total) * 100 || 0;
        const cPct = (anime.completed / total) * 100 || 0;
        const dPct = (anime.dropped / total) * 100 || 0;
        const pPct = (anime.plan / total) * 100 || 0;

        let matchPct = Math.min(Math.max(anime.score * 100, 0), 99.9);
        const trailerHtml = anime.trailer ? `<a href="${anime.trailer}" target="_blank" class="btn-trailer">Watch Trailer</a>` : '';
        const coverImage = anime.picture || 'https://via.placeholder.com/150x220?text=No+Cover';
        const genreTags = Array.isArray(anime.genres)
            ? anime.genres.map(g => `<span class="genre-chip">${typeof g === 'string' ? g : g.name || g}</span>`).join(' ')
            : '';

        const html = `
            <div class="card ${isGold}" style="animation-delay: ${i * 0.1}s">
                ...
                <div class="info">
                    <div style="padding-right: 90px;">
                        <h2><a href="https://myanimelist.net/anime/${anime.id}" target="_blank" title="${anime.title}">${anime.title}</a></h2>
                        ...
                        <div style="font-size: 0.9rem; color: #fbbf24; font-weight: 800; display:flex; align-items:center; gap:6px; margin-top: 0.6rem;">
                            ★ ${anime.raw_mean_score.toFixed(2)} <span style="color:var(--text-muted); font-weight:normal; font-size:0.8rem;">(MAL Score)</span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="stats-grid">
                            <div class="badge"><span style="background:#10b981"></span> ${anime.watching.toLocaleString()} Watch</div>
                            <div class="badge"><span style="background:#3b82f6"></span> ${anime.completed.toLocaleString()} Done</div>
                            <div class="badge"><span style="background:#f43f5e"></span> ${anime.dropped.toLocaleString()} Drop</div>
                            <div class="badge"><span style="background:#a855f7"></span> ${anime.plan.toLocaleString()} Plan</div>
                        </div>
                    </div>
                </div>
